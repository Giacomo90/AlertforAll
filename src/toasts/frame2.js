...
export function frame2({TitleMsg,Ico},sett){
  ...
  const styles={
    ...
    Toast:{
     position:'fixed',
     bottom: setting.positionVertical ? (setting.positionVertical === 'center' ? '50%' : setting.positionVertical === 'start' ? null : '8%' ) : '8%',
     top: setting.positionVertical === 'start' ? '8%' : null,
    ...
    TitleTxt:{
        marginTop:'10px',
        color:Ico === 'Success' ? '#5CC131' : Ico === 'Error' ? '#FF3131' : Ico === 'Warning' ? '#FFBD59' : Ico === 'Info' ? '#0CC0DF' : null
    }
   }
